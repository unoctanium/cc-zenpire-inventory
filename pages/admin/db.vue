...
async function runExport() {
  ...
  try {
    ...
    toast.add({ title: t('devTools.exportFailed'), description: e?.data?.statusMessage ?? e?.message ?? String(e), color: 'error' })
  } finally {
  ...
  reader.onload = (e) => {
    try {
      ...
      for (const tbl of expected) {
        if (!Array.isArray(parsed.tables[tbl])) throw new Error(`Missing table: ${tbl}`)
      }
...
async function confirmImport() {
  ...
  try {
    await $fetch('/api/manage/import', { method: 'POST', credentials: 'include', body: importParsed.value })
    toast.add({ title: t('devTools.importSuccess') })
    importModalOpen.value = false
    importParsed.value    = null
  } catch (e: any) {
    toast.add({ title: t('devTools.importFailed'), description: e?.data?.statusMessage ?? e?.message ?? String(e), color: 'error' })
  } finally {
          ...
          <table class="text-sm">
            <tbody>
              <tr>
                ...
                <td class="py-1 align-middle text-gray-600 dark:text-gray-400">{{ $t('devTools.purgeDesc') }}</td>
              </tr>
              <tr>
                <td class="pr-4 py-1 align-middle">
                ...
                <td class="py-1 align-middle text-gray-600 dark:text-gray-400">{{ $t('devTools.seedInitialDesc') }}</td>
              </tr>
              <tr>
                <td class="pr-4 py-1 align-middle">
                ...
                <td class="py-1 align-middle text-gray-600 dark:text-gray-400">{{ $t('devTools.seedExampleDesc') }}</td>
              </tr>
            </tbody>
          </table>
          ...
          <table class="text-sm">
            <tbody>
              <tr>
                ...
                <td class="py-1 align-middle text-gray-600 dark:text-gray-400">{{ $t('devTools.dbImportDesc') }}</td>
              </tr>
              <tr>
                <td class="pr-4 py-1 align-middle">
                ...
                <td class="py-1 align-middle text-gray-600 dark:text-gray-400">{{ $t('devTools.dbExportDesc') }}</td>
              </tr>
              <tr>
                <td class="pr-4 py-1 align-middle">
                ...
                <td class="py-1 align-middle text-gray-600 dark:text-gray-400">{{ $t('devTools.dbExportPlainDesc') }}</td>
              </tr>
            </tbody>
          </table>
            ...
            <template #body>
              <div class="space-y-3 text-sm">
                ...
                <ul class="list-disc list-inside text-gray-600 dark:text-gray-400 space-y-0.5">
                  <li v-for="(count, table) in importSummary" :key="table">{{ table }}: {{ count }}</li>
                </ul>
              </div>
            </template>
            <template #footer>
              <div class="flex justify-end gap-2">
                <UButton color="neutral" variant="soft" @click="importModalOpen = false">{{ $t('common.cancel') }}</UButton>
                <UButton color="error" variant="solid" :loading="importing" @click="confirmImport">{{ $t('devTools.importConfirm') }}</UButton>
              </div>
            </template>
          </UModal>
        </div>
