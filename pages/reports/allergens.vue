...
watchEffect(() => {
  rows.value = data.value?.allergens ?? []
})
    ...
    <template #table>
      <div ref="tableContainer">
        ...
        >
          ...
          <tbody>
            <tr v-if="pending">
              <td colspan="2" class="px-2 py-2 text-gray-500 dark:text-gray-400">{{ $t('common.loading') }}</td>
            </tr>
            <tr v-else-if="visibleRows.length === 0">
              <td colspan="2" class="px-2 py-2 text-gray-500 dark:text-gray-400">{{ $t('common.noData') }}</td>
            </tr>
