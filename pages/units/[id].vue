...
watch(unit, (u) => {
  if (u) { draft.code = u.code; draft.name = u.name; draft.unit_type = u.unit_type; draft.factor = u.factor }
}, { immediate: true })
...
async function doDelete() {
  try {
    await $fetch(`/api/units/${id.value}`, { method: 'DELETE', credentials: 'include' })
...
<template>
  <div>
    <div class="sm:hidden flex items-center gap-2 px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
      <button class="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400" @click="navigateTo('/units')">
        <UIcon name="i-heroicons-chevron-left" class="w-5 h-5" />
        {{ $t('nav.units') }}
      </button>
    </div>

    <div v-if="unit" class="p-4 space-y-4 max-w-sm">

      <div class="flex items-center justify-between gap-2 pb-2 border-b border-gray-200 dark:border-gray-800">
        <h2 class="text-base font-semibold text-gray-900 dark:text-gray-100">
          {{ editMode ? $t('common.edit') : unit.code + ' — ' + unit.name }}
        </h2>
